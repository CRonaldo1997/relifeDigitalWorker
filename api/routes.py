...
_LOGIN_LOCALE = {
    "en": {
        "lang": "en",
        "title": "Welcome back",
        "subtitle": "Access your Hermes control center",
        "btn": "Get started",
        "invalid_pw": "Invalid username or password",
        "conn_failed": "Connection failed, please try again",
    },
    "zh": {
        "lang": "zh-CN",
        "title": "欢迎回来",
        "subtitle": "欢迎访问数字员工控制中心",
        "btn": "由此开启",
        "invalid_pw": "用户名或密码错误",
        "conn_failed": "连接失败，请重试",
    },
    "zh-Hant": {
        "lang": "zh-TW",
        "title": "歡迎回來",
        "subtitle": "訪問您的數位員工1號控制中心",
        "btn": "由此開啟",
        "invalid_pw": "用戶名或密碼錯誤",
        "conn_failed": "連接失敗，請重试",
    },
}
...
# ── Shared CSS for login/register pages ──────────────────────────────────────
_AUTH_CSS = """
*{box-sizing:border-box;margin:0;padding:0}
html,body{height:100%;min-height:100dvh;
  font-family:'Outfit','PingFang SC','Microsoft YaHei',system-ui,sans-serif;
  background:#e8eff6;color:#0D1B2E;overflow:hidden;}
body{
  display:flex;align-items:stretch;
  background:url('/static/chat_bg.png') center/cover no-repeat;
  position:relative;
}
/* semi-transparent white overlay on left side to keep card readable */
body::before{
  content:'';position:absolute;inset:0;pointer-events:none;
  background:linear-gradient(95deg,rgba(255,255,255,.82) 0%,rgba(255,255,255,.55) 38%,rgba(255,255,255,0) 62%);
}
.left-panel{
  width:min(760px,70%);display:flex;flex-direction:column;justify-content:center;
  align-items:center;padding:60px 40px;position:relative;z-index:1;
}
.brand-label{
  width:min(520px,100%);display:block;font-size:16px;font-weight:700;letter-spacing:.25em;
  text-transform:uppercase;color:rgba(0,87,163,.7);margin-bottom:16px;text-align:left;
}
.brand-title{
  width:min(520px,100%);display:block;font-size:clamp(36px,4.5vw,52px);font-weight:900;
  color:#0D1B2E;letter-spacing:-.03em;line-height:1.1;margin-bottom:0;text-align:left;
}
/* card sits inside .left-panel, centered by align-items:center */
.card{
  margin-top:44px;
  width:min(520px,100%);
  background:rgba(255,255,255,.94);
  border:1px solid rgba(0,87,163,.14);border-radius:36px;padding:60px 60px 48px;
  box-shadow:0 20px 50px -12px rgba(0,87,163,.18),0 40px 90px -25px rgba(0,87,163,.28),
    0 0 0 1px rgba(255,255,255,1) inset;
  backdrop-filter:blur(35px);-webkit-backdrop-filter:blur(35px);
}
.logo-row{width:100%;display:flex;align-items:center;justify-content:center;gap:20px;margin-bottom:40px;}
.logo-img{height:84px;width:auto;object-fit:contain;flex-shrink:0;}
.logo-text{display:flex;flex-direction:column;gap:1px;}
.logo-name{font-size:14px;font-weight:700;color:#0D1B2E;letter-spacing:-.01em;line-height:1.25;}
.logo-tagline{font-size:9px;font-weight:600;color:#9DAFC0;text-transform:uppercase;letter-spacing:.1em;}
h2{width:100%;font-size:28px;font-weight:800;color:#0D1B2E;letter-spacing:-.02em;margin-bottom:14px;text-align:center;}
.sub{width:100%;font-size:16px;color:#7A8DA0;margin-bottom:44px;line-height:1.5;text-align:center;}
.field{margin-bottom:26px;}
.field-label{display:block;font-size:10px;font-weight:700;color:#9DAFC0;
  text-transform:uppercase;letter-spacing:.1em;margin-bottom:5px;}
input[type=text],input[type=password]{
  width:100%;padding:10px 13px;border-radius:10px;
  border:1.5px solid rgba(0,87,163,.14);background:rgba(240,244,248,.8);
  color:#0D1B2E;font-size:13px;font-family:inherit;outline:none;
  transition:border-color .18s,box-shadow .18s,background .18s;
}
input[type=text]::placeholder,input[type=password]::placeholder{color:#B5C4D0;}
input[type=text]:focus,input[type=password]:focus{
  border-color:#0057A3;background:rgba(255,255,255,.98);
  box-shadow:0 0 0 3px rgba(0,87,163,.1);
}
.submit-btn{
  width:100%;padding:11px;border-radius:10px;border:none;
  background:linear-gradient(135deg,#0057A3 0%,#0284C7 100%);
  color:#fff;font-size:13px;font-weight:700;font-family:inherit;cursor:pointer;
  transition:all .2s cubic-bezier(.16,1,.3,1);letter-spacing:.01em;margin-top:6px;
  box-shadow:0 4px 12px rgba(0,87,163,.28);
}
.submit-btn:hover{background:linear-gradient(135deg,#004A8F,#0272B4);
  transform:translateY(-1px);box-shadow:0 6px 18px rgba(0,87,163,.36);}
...
.switch-link{text-align:center;margin-top:14px;font-size:12px;color:#7A8DA0;}
.switch-link a{color:#0057A3;text-decoration:none;font-weight:600;}
.switch-link a:hover{text-decoration:underline;}
@media(max-width:640px){
  body::before{background:rgba(255,255,255,.75);}
  .left-panel{width:100%;padding:28px 20px;align-items:center;}
  .card{margin-top:20px;width:100%;}
}
"""
...
<div class="left-panel">
  <div class="brand-label">DIGITAL EMPLOYEE NO. 1</div>
  <div class="brand-title">{{BOT_NAME}}</div>
  <div class="card">
    <div class="logo-row">
      <img class="logo-img" src="/static/logo.png" alt="{{BOT_NAME}} logo">
    </div>
    <h2>{{LOGIN_TITLE}}</h2>
    <p class="sub">{{LOGIN_SUBTITLE}}</p>
    <form id="login-form" data-conn-failed="{{LOGIN_CONN_FAILED}}">
      <div class="field">
        <label class="field-label" for="username">账户名称</label>
        <input type="text" id="username" name="username" placeholder="请输入用户名" autocomplete="username" autofocus>
      </div>
      <div class="field">
        <label class="field-label" for="pw">账号密码</label>
        <input type="password" id="pw" name="password" placeholder="请输入密码" autocomplete="current-password">
      </div>
      <button class="submit-btn" type="submit">{{LOGIN_BTN}}</button>
    </form>
    <div class="err" id="err"></div>
    <div class="switch-link">
      <a href="/register">创建您的数字员工通行证</a>
    </div>
  </div>
</div>
...
<div class="left-panel">
  ...
  <div class="card">
    ...
    <h2>注册账号</h2>
    <p class="sub">创建您的数字员工通行证</p>
    <form id="register-form">
      <div class="field">
        <label class="field-label" for="reg-username">用户名</label>
        <input type="text" id="reg-username" name="username" placeholder="请输入用户名" autocomplete="username" autofocus>
      </div>
      <div class="field">
        <label class="field-label" for="reg-pw">密码</label>
        <input type="password" id="reg-pw" name="password" placeholder="请输入密码（至少4位）" autocomplete="new-password">
      </div>
      <div class="field">
        <label class="field-label" for="reg-pw2">确认密码</label>
        <input type="password" id="reg-pw2" name="password2" placeholder="再次输入密码" autocomplete="new-password">
      </div>
...
def handle_get(handler, parsed) -> bool:
    ...
    if parsed.path == "/login":
        ...
        from api.user_auth import parse_cookie as _ua_parse_cookie, verify_session as _ua_verify
        cv = _ua_parse_cookie(handler)
        if cv and _ua_verify(cv):
    ...
    if parsed.path == "/api/auth/status":
        from api.user_auth import parse_cookie as _ua_parse_cookie, verify_session as _ua_verify
        cv = _ua_parse_cookie(handler)
        session_info = _ua_verify(cv) if cv else None
        logged_in = session_info is not None
        username = session_info["username"] if session_info else ""
        return j(handler, {"auth_enabled": True, "logged_in": logged_in, "username": username})

    if parsed.path == "/api/auth/user_count":
        from api.user_auth import get_user_count
        return j(handler, {"count": get_user_count()})
...
def handle_post(handler, parsed) -> bool:
    ...
    if parsed.path == "/api/profile/create":
        ...
        if not _re.match(r"^[a-z0-9\u4e00-\u9fa5][a-z0-9\u4e00-\u9fa5_-]{0,63}$", name, _re.IGNORECASE):
            return bad(
                handler,
        ...
        if clone_from is not None:
            ...
            if not _re.match(r"^[a-z0-9\u4e00-\u9fa5][a-z0-9\u4e00-\u9fa5_-]{0,63}$", clone_from, _re.IGNORECASE):
    ...
    if parsed.path == "/api/auth/login":
        from api.user_auth import (
            authenticate_user,
            create_session as _ua_create_session,
            set_auth_cookie as _ua_set_cookie,
            _check_login_rate,
            _record_login_attempt,
        )
        client_ip = handler.client_address[0]
        if not _check_login_rate(client_ip):
            return j(handler, {"error": "请求过于频繁，请稍后再试"}, status=429)
        username = body.get("username", "").strip()
        password = body.get("password", "")
        ok, result = authenticate_user(username, password)
        if not ok:
            _record_login_attempt(client_ip)
            return bad(handler, result, 401)
        cookie_val = _ua_create_session(result)  # result = canonical username
        handler.send_response(200)
        handler.send_header("Content-Type", "application/json")
        handler.send_header("Cache-Control", "no-store")
        _security_headers(handler)
        _ua_set_cookie(handler, cookie_val)
        handler.end_headers()
        handler.wfile.write(json.dumps({"ok": True, "username": result}).encode())
        return True

    if parsed.path == "/api/auth/register":
        from api.user_auth import register_user
        username = body.get("username", "").strip()
        password = body.get("password", "")
    ...
    if parsed.path == "/api/auth/logout":
        from api.user_auth import (
            clear_auth_cookie as _ua_clear_cookie,
            invalidate_session as _ua_invalidate,
            parse_cookie as _ua_parse_cookie,
        )
        cookie_val = _ua_parse_cookie(handler)
        if cookie_val:
            _ua_invalidate(cookie_val)
        handler.send_response(200)
        handler.send_header("Content-Type", "application/json")
        handler.send_header("Cache-Control", "no-store")
        _security_headers(handler)
        _ua_clear_cookie(handler)
        handler.end_headers()
