...
This module is the authoritative auth layer when HERMES_WEBUI_USER_AUTH=1.
When disabled the existing single-password auth (api/auth.py) is used.
"""
import hashlib
import hmac
import http.cookies
...
# Rate limiter (shared with existing auth.py pattern)
...
def _get_db() -> sqlite3.Connection:
    ...
    Handles schema migration from previous versions that used a
    separate 'salt' column and 'profile_name' column.
    """
    ...
    if cols and "salt" in cols:
        # Old schema detected — migrate: backup old data, recreate table
        try:
            old_users = conn.execute("SELECT username FROM users").fetchall()
            conn.execute("DROP TABLE IF EXISTS users")
            conn.commit()
            logger.info("Migrated users table schema (dropped old salt/profile_name columns). %d users need to re-register.", len(old_users))
...
# ── Auth check (drop-in for api/auth.check_auth) ─────────────────────────────
