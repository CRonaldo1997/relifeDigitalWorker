...
async function getSlashAutocompleteMatches(text){
  ...
  if(parsed.kind==='commands') {
    ...
    return getMatchingCommands(parsed.query);
  }
...
function _skillCommandSlug(name){
  ...
  if(!raw)return'';
  // Support Unicode (Chinese etc.) in skill slugs so they appear in the / dropdown.
  // We replace spaces/underscores with hyphens and strip only punctuation/symbols,
  // preserving any letter or digit from any language.
