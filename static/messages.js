...
function attachLiveStream(activeSid, streamId, uploaded=[], options={}){
  ...
  function ensureAssistantRow(force=false){
    ...
    if(!force&&!assistantRow){
      const parsed=_getParsedState();
      if(!parsed || !String(parsed.displayText||'').trim()) return;
    }
  ...
  function _smdWrite(displayText){
    ...
    // from anchors/images that were just added to the live DOM.
    //
    // Performance optimization: throttling querySelectorAll calls to every 500ms
    // during streaming.  The final 'done' handler runs a final full sanitization.
    if(assistantBody){
      const now=performance.now();
      if(now - _lastSanitizeMs > 500){
        _lastSanitizeMs=now;
  ...
  function _wireSSE(source){
    ...
    source.addEventListener('done',e=>{
      ...
      if(assistantBody){
        ...
        _smdEndParser();
        // Ensure final sanitization runs even if the last throttled run was a while ago
        _sanitizeSmdLinks(_finBody);
