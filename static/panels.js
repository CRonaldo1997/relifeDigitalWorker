...
function syncAppTitlebar() {
  ...
  if (panel === 'chat') {
    if (typeof S !== 'undefined' && S && S.session) {
      ...
      mainText = window._botName || 'Hermes';
    }
  } else {
...
function _renderProfileDetail(p, activeName){
  ...
  const statusBadge = isActive
    ? `<span class="detail-badge active">${esc(t('profile_active'))}</span>`
    : `<span class="detail-badge">未激活</span>`;
  const defaultBadge = isDefault ? ` <span class="detail-badge">${esc(t('profile_default_label'))}</span>` : '';
  ...
  const rows = [];
  rows.push(`<div class="detail-row"><div class="detail-row-label">状态</div><div class="detail-row-value">${statusBadge}${defaultBadge}</div></div>`);
  rows.push(`<div class="detail-row"><div class="detail-row-label">网关</div><div class="detail-row-value">${gwBadge}</div></div>`);
  if (p.model) rows.push(`<div class="detail-row"><div class="detail-row-label">模型</div><div class="detail-row-value"><code>${esc(p.model)}</code></div></div>`);
  if (p.provider) rows.push(`<div class="detail-row"><div class="detail-row-label">提供商</div><div class="detail-row-value">${esc(p.provider)}</div></div>`);
  if (p.base_url) rows.push(`<div class="detail-row"><div class="detail-row-label">基础 URL</div><div class="detail-row-value"><code>${esc(p.base_url)}</code></div></div>`);
  rows.push(`<div class="detail-row"><div class="detail-row-label">API 密钥</div><div class="detail-row-value">${p.has_env ? esc(t('profile_api_keys_configured')) : '<span style="color:var(--muted)">未配置</span>'}</div></div>`);
  if (typeof p.skill_count === 'number') rows.push(`<div class="detail-row"><div class="detail-row-label">技能</div><div class="detail-row-value">${esc(t('profile_skill_count', p.skill_count))}</div></div>`);
  if (p.default_workspace) rows.push(`<div class="detail-row"><div class="detail-row-label">默认空间</div><div class="detail-row-value"><code>${esc(p.default_workspace)}</code></div></div>`);
  body.innerHTML = `
    <div class="main-view-content">
      <div class="detail-card">
        <div class="detail-card-title">数字员工</div>
        ${rows.join('')}
      </div>
    </div>`;
  body.style.display = '';
...
async function saveProfileForm(){
  ...
  if (!/^[a-z0-9\u4e00-\u9fa5][a-z0-9\u4e00-\u9fa5_-]{0,63}$/i.test(name)) { errEl.textContent = t('profile_name_rule'); errEl.style.display = ''; return; }
...
function switchSettingsSection(name){
  const section=(name==='appearance'||name==='preferences'||name==='providers'||name==='system'||name==='welcome')?name:'conversation';
  _settingsSection=section;
  _currentSettingsSection=section;
  const map={conversation:'Conversation',appearance:'Appearance',preferences:'Preferences',providers:'Providers',system:'System',welcome:'Welcome'};
  // Sidebar menu items
  document.querySelectorAll('#settingsMenu .side-menu-item').forEach(it=>{
    it.classList.toggle('active', it.dataset.settingsSection===section);
  });
  // Panes in main
  ['conversation','appearance','preferences','providers','system','welcome'].forEach(key=>{
  ...
  if(section==='welcome') renderSuggestionsManager();
}
...
async function loadSettingsPanel(){
  try{
    ...
    // Suggestions
    window._welcomeSuggestions = Array.isArray(settings.welcome_suggestions) ? settings.welcome_suggestions : [];
    ...
    if(_settingsSection==='welcome') renderSuggestionsManager();
...
function renderSuggestionsManager() {
  const list = $('suggestionsManagerList');
  if (!list) return;
  list.innerHTML = '';
  const suggestions = window._welcomeSuggestions || [];
  suggestions.forEach((s, idx) => {
    const row = document.createElement('div');
    row.className = 'suggestion-manager-row';
    ...
    input.value = s;
    input.placeholder = t('suggestion_placeholder');
    input.style.flex = '1';
    ...
    input.oninput = (e) => {
      window._welcomeSuggestions[idx] = e.target.value;
      _markSettingsDirty();
    };
    ...
    delBtn.title = t('delete_suggestion');
    delBtn.onclick = () => {
      window._welcomeSuggestions.splice(idx, 1);
      _markSettingsDirty();
      renderSuggestionsManager();
    };
...
function addWelcomeSuggestion() {
  if (!window._welcomeSuggestions) window._welcomeSuggestions = [];
  window._welcomeSuggestions.push('');
  _markSettingsDirty();
  renderSuggestionsManager();
  // Focus the new input
  setTimeout(() => {
    const inputs = document.querySelectorAll('#suggestionsManagerList input');
...
async function saveSettings(andClose){
  ...
  // Welcome suggestions
  if (window._welcomeSuggestions) {
    body.welcome_suggestions = window._welcomeSuggestions.filter(s => s.trim() !== '');
  }
