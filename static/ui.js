...
function saveInflightState(sid, state){
  if(!sid||!state) return;
  try{
    const key = `hermes-inflight-state-${sid}`;
    localStorage.setItem(key, JSON.stringify({...state, updated_at:Date.now()}));
  }catch(_){ }
}
...
function clearInflightState(sid){
  if(!sid) return;
  try{
    localStorage.removeItem(`hermes-inflight-state-${sid}`);
  }catch(_){ }
}

// Cleanup: remove the legacy monolithic inflight state key once if it exists.
// This key was replaced by per-session keys for performance.
try {
  if (localStorage.getItem(INFLIGHT_STATE_KEY)) {
...
function _assistantRoleHtml(tsTitle=''){
  ...
  return `<div class="msg-role assistant" ${tsTitle?`title="${esc(tsTitle)}"`:''}><div class="role-icon assistant"><img src="static/ai.png" class="role-icon-img" alt=""></div><span style="font-size:12px">${esc(_bn)}</span></div>`;
}
function _userRoleHtml(tsTitle=''){
  const _un=localStorage.getItem('hermes-username')||'User';
  return `<div class="msg-role user" ${tsTitle?`title="${esc(tsTitle)}"`:''}><span style="font-size:12px">${esc(_un)}</span><div class="role-icon user"><img src="static/user.png" class="role-icon-img" alt=""></div></div>`;
}
...
function renderMessages(){
  ...
  if(isEmpty && typeof renderWelcomeSuggestions==='function') renderWelcomeSuggestions();
  inner.innerHTML='';
...
function renderWelcomeSuggestions() {
  ...
  suggestions.forEach(s => {
    ...
    btn.dataset.msg = s;
    btn.innerHTML = `<span>${esc(s)}</span><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="5" y1="12" x2="19" y2="12"></line><polyline points="12 5 19 12 12 19"></polyline></svg>`;
    btn.onclick = () => {
      const ta = $('msg');
      if (ta) {
        ta.value = s;
