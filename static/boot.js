...
function syncWorkspacePanelUI(){
  ...
  if(toggleBtn){
    ...
    toggleBtn.title=isOpen?t('hide_workspace_panel'):t('show_workspace_panel');
    toggleBtn.disabled=!canBrowse;
  }
  if(collapseBtn){
    collapseBtn.title=isCompact?t('close_workspace_panel'):t('hide_workspace_panel');
  }
  ...
  if(clearBtn){
    ...
    clearBtn.title=hasPreview?t('close_preview'):t('hide_workspace_panel');
...
(function(){
  ...
  function _setRecording(on){
    ...
    if(statusText) statusText.textContent=on?'正在聆听':'正在聆听';
...
function applyBotName(){
  ...
  const msg=$('msg');
  if(msg) msg.placeholder=t('msg_placeholder_named', name);
}
...
(async()=>{
  ...
  if(typeof renderWelcomeSuggestions==='function') renderWelcomeSuggestions();
  await renderSessionList();
  // Start real-time gateway session sync if setting is enabled
  if(typeof startGatewaySSE==='function') startGatewaySSE();
  // Pre-load skills for autocomplete to avoid lag on first "/" press
  if(typeof ensureSkillCommandsLoadedForAutocomplete==='function') ensureSkillCommandsLoadedForAutocomplete();
})();
